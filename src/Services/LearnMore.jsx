...
export default function LearnMore(props) {
  ...
  const { serviceArr: data, setServiceArr, isAnyTherapyBooked, setIsAnyTherapyBooked } = useContext(MyContext);
  ...
  useEffect(() => {
    ...
    if (bookedDate === today) {
      ...
      setServiceArr(updatedServices);
    }
  }, [bookedDate, data, setServiceArr]);
  ...
  const parseDuration = (rawDuration) => {
    ...
    if (Number(durationTokens[0]) > 0) {
      parsedDuration += durationTokens[0] + " hr ";
    }
  ...
  return (
    <div className="HomeContainer">
      <ScrollToTop />
      {data && data.filter((service) => location.state.apptID === service.treatment_uid).map((filteredService) => (
        <div key={filteredService.treatment_uid} style={{ padding: "3% 20% 3% 20%" }}>
          <div style={{ textAlign: "center" }}>
            <div className="LearnMoreTitle">{filteredService.title}</div>
            <div className="LearnMoreText">{filteredService.description} <br /></div>
            <div className="LearnMoreHeader">{parseDuration(filteredService.duration)} | {filteredService.cost}</div>
            <div style={{ fontWeight: "300", marginTop: "1rem" }}>
              Already booked? Fill out this waiver.
            </div>
            <WaiverBtn />
            ...
            <BookNowBTN 
              ...
              isTherapyBooked={isAnyTherapyBooked}
            />
